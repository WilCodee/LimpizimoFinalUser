...
const ServiceAccepted = props => {
  ...
  const exampleItems = [
    {
      id: 1,
      title: 'Plus',
      text: 'Nuestra colaboradora realizó más de 200 servicios de 5 estrellas.',
  ...
  const renderItem = useCallback(({ item, index }) => (
    ...
    >
      <Text style={{ fontSize: 18, color: 'white', paddingLeft: 20, paddingTop: 10 }}>{item.title}</Text>
      <Text style={{ fontSize: 12, color: 'white', paddingLeft: 20, textAlign: 'left', width: 250 }}>{item.text}</Text>
  ...
  const service = {
    ...
    cleaningOption: strings('common.cleaning.normal'),
    selectedPlace: {
      id: 1,
      callePrincipal: 'Av 3 de abril'
    },
    selectedCard: {
      id: 1,
      name: 'Visa',
  ...
  const employee = {
    name: 'Sarah Potter',
    servicesAmount: 55,
    imageURL: 'https://smashinglogo.com/static/img/virtual-designers/peter.gif',
    rating: 4.89,
